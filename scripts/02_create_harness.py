...
运行：
    /usr/bin/python3.11 opc_copilot/scripts/02_create_harness.py
"""
...
def main() -> int:
    ...
    if existing:
        ...
        detail = wait_harness_ready(control, harness_id)

        # 服务端实际挂载的 skill 名（从 S3 URI 尾段取），引导节据此生成
        server_skill_names = [
            s["s3"]["uri"].rstrip("/").split("/")[-1]
        ...
        created = control.create_harness(
            ...
            # baseline：不挂 skill，引导节不出现，提示词与原版一致
    ...
    if state["phase"] == 1:
        print("\n下一步（首次创建）：")
        print("  /usr/bin/python3.11 opc_copilot/scripts/03_chat.py              交互对话")
        print("  /usr/bin/python3.11 opc_copilot/scripts/04_run_evalset.py       跑评测集，记录 baseline 分数")
    else:
        print("\n下一步：")
        print("  /usr/bin/python3.11 opc_copilot/scripts/04_run_evalset.py       重跑评测")
