...
运行：
    /usr/bin/python3.11 opc_copilot/scripts/08_update_harness_tools.py --phase 2
    /usr/bin/python3.11 opc_copilot/scripts/08_update_harness_tools.py --phase 3
    /usr/bin/python3.11 opc_copilot/scripts/08_update_harness_tools.py --phase 5
"""
...
def main() -> int:
    ...
    if phase >= 5 and not gateway_state.get("forbid_policy_ids"):
        print(
            ...
            "  请接着运行：/usr/bin/python3.11 opc_copilot/scripts/10_setup_policy.py"
        )
    ...
    # 不等 READY 就跑评测，放开的工具还没生效，测出来的还是旧配置的分数
    ...
    print("\n下一步：")
    print("  /usr/bin/python3.11 opc_copilot/scripts/03_chat.py           手动验证工具确实被调用")
    print("  /usr/bin/python3.11 opc_copilot/scripts/04_run_evalset.py    跑评测，看与上一 Phase 的分数差")
