...
--------------------------------------------------------
运行：
    /usr/bin/python3.11 opc_copilot/scripts/09_enable_memory.py
    /usr/bin/python3.11 opc_copilot/scripts/09_enable_memory.py --disable    关掉，回到无记忆状态
"""
...
import boto3
from common import HARNESS_STATE_FILE, load_harness_state, save_json, wait_harness_ready
...
def main() -> int:
    ...
    if args.disable:
        ...
        wait_harness_ready(control, harness_id)
    ...
    # 等 Harness 回到 READY：UpdateHarness 是异步的，受理 ≠ 生效。
    # 但注意这之后托管 Memory 实例本身还要几分钟才 ACTIVE，
    # 那是另一层等待（见下方提示），两者别混为一谈。
    wait_harness_ready(control, harness_id)
    print("✓ 已提交（状态 READY）")
    ...
    print("\n然后跑评测：")
    print("  /usr/bin/python3.11 opc_copilot/scripts/04_run_evalset.py                  用例间互相隔离（默认）")
    print("  /usr/bin/python3.11 opc_copilot/scripts/04_run_evalset.py --no-isolate     让记忆跨用例累积")
    return 0
