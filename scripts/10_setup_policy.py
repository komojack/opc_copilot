...
运行：
    /usr/bin/python3.11 opc_copilot/scripts/10_setup_policy.py
    /usr/bin/python3.11 opc_copilot/scripts/10_setup_policy.py --verify    只做网关直连拦截预检
"""
...
FORBID_RULES = [
    # 阈值对应续约规则中的 20 万上限：合同年额 > 20 万需创始人确认。
    # 原值 50000 错误地拦截了 9.6 万等正常续约案例（EV-12 归因：threshold）。
    # 改为 200001（> 200000）与 renewal_rules.md 中"合同年额超 20 万升级"一致。
...
when {
    context.input has amount && context.input.amount > 200000
};"""),
...
VERIFY_CASES = [
    ...
    # 续约年额 9.6 万（EV-12 场景）：未超 20 万，应放行（原值 50000 会误拦）
    ("续约年额 9.6 万，未超 20 万，应放行", {
        "entity_id": "CLIENT-ECHO", "decision_type": "renewal",
        "summary": "回声传媒续约维持 5% 折扣，年额 96000", "amount": 96000,
        "is_new_counterparty": False, "deviates_from_precedent": False,
        "counterparty_has_overdue": False,
    }, False),
    ("金额 21 万超 20 万阈值，应拦截", {
        "entity_id": "CLIENT-DELTA", "decision_type": "renewal",
        "summary": "德尔塔续约年额 21 万", "amount": 210000,
        "is_new_counterparty": False, "deviates_from_precedent": False,
        "counterparty_has_overdue": False,
    }, True),
    ("新客户首单，应拦截", {
...
def main() -> int:
    ...
    print("\n务必先做直连预检，再跑对话测试：")
    print("  /usr/bin/python3.11 opc_copilot/scripts/10_setup_policy.py --verify")
    print("\n预检通过后放开 record_decision：")
    print("  /usr/bin/python3.11 opc_copilot/scripts/08_update_harness_tools.py --phase 5")
