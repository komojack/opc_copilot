...
--------------------------------------------------------
运行：
    /usr/bin/python3.11 opc_copilot/scripts/00_validate_data.py
"""
...
def check_evalset(entity_ids: dict) -> None:
    ...
    # expected_skills 只能引用本地 skills/ 下真实存在的 skill
    ...
    for case in cases:
        ...
        # expected_skills 同理：skill 触发断言（skill_accuracy）要读它
        if "expected_skills" not in case:
            errors.append(f"用例 {cid} 缺少字段 expected_skills（无期望 skill 时给空列表 []）")
        elif not isinstance(case["expected_skills"], list):
            errors.append(f"用例 {cid} 的 expected_skills 必须是列表，当前是 {type(case['expected_skills']).__name__}")
        else:
            # 与本地 skills/ 目录对得上，写错名字的话触发率统计永远 0，很难排查
            for sk in case["expected_skills"]:
                if sk not in known_skills:
                    errors.append(
                        f"用例 {cid} 的 expected_skills 引用了不存在的 skill：{sk} "
                        f"（本地 skills/ 下有：{', '.join(sorted(known_skills)) or '无'}）"
                    )
