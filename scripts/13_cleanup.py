...
def main() -> int:
    ...
    if mcp:
        ...
        # toolkit 自建角色 + CodeBuild 源码桶：auto_create_execution_role=True 时
        # toolkit 会自建 AmazonBedrockAgentCoreSDKRuntime-* 和
        # AmazonBedrockAgentCoreSDKCodeBuild-* 两个角色（不在 CFN 栈里，删栈删不掉），
        # 以及 bedrock-agentcore-codebuild-sources-* 源码桶。13 负责清理。
        # 本次实验只删这一批——CodeBuild 角色从 .bedrock_agentcore.yaml 读，
        # Runtime 角色按"同批部署后缀相同"原则匹配（后缀 = CodeBuild 角色名尾部）。
        print("\n[3b] toolkit 自建角色与源码桶（本次实验残留）")
        yaml_path = os.path.join(PROJECT_ROOT, "mcp_server", ".bedrock_agentcore.yaml")
        iam = boto3.client("iam")
        s3_cb = boto3.client("s3", region_name=region)
        toolkit_codebuild_role = None
        source_bucket = None
        if os.path.exists(yaml_path):
            try:
                ...
                agent_cfg = cfg.get("agents", {}).get(mcp.get("agent_name", ""), {})
                toolkit_codebuild_role = agent_cfg.get("codebuild", {}).get("execution_role")
                source_bucket = agent_cfg.get("codebuild", {}).get("source_bucket")
            except Exception as e:
                print(f"  ⚠ 读 .bedrock_agentcore.yaml 失败：{type(e).__name__}")

        # CodeBuild 源码桶：先清空再删
        if source_bucket:
            step(f"清空 CodeBuild 源码桶 {source_bucket}",
                 lambda b=source_bucket: empty_bucket(s3_cb, b))
            step(f"删除 CodeBuild 源码桶 {source_bucket}",
                 lambda b=source_bucket: s3_cb.delete_bucket(Bucket=b))

        # CodeBuild 角色
        if toolkit_codebuild_role:
            cb_role_name = toolkit_codebuild_role.split("/")[-1]
            step(f"toolkit CodeBuild 角色 {cb_role_name}",
                 lambda n=cb_role_name: iam.delete_role(RoleName=n))

            # Runtime 角色：同批部署后缀与 CodeBuild 角色相同，按后缀匹配只删本次那个
            suffix = cb_role_name.rsplit("-", 1)[-1]
            prefix = f"AmazonBedrockAgentCoreSDKRuntime-{region}-"
            try:
                for page in iam.get_paginator("list_roles").paginate():
                    for r in page["Roles"]:
                        rn = r["RoleName"]
                        if rn.startswith(prefix) and rn.endswith(suffix):
                            step(f"toolkit Runtime 角色 {rn}",
                                 lambda n=rn: iam.delete_role(RoleName=n))
            except Exception as e:
                print(f"  ⚠ 列举 toolkit Runtime 角色失败：{type(e).__name__}")

        # 删 .bedrock_agentcore.yaml（toolkit 配置残留）
        step("mcp_server/.bedrock_agentcore.yaml",
             lambda: os.remove(yaml_path))
    ...
    print("\n最后删 CloudFormation 栈（知识库、向量桶、IAM 角色随栈删除）：")
    print(f"  aws cloudformation delete-stack --stack-name {env.get('cfn_stack_name', 'agentcore')}")
    print("  # 若另有其他实验栈，按各自栈名分别删")
